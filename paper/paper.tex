...
Five named profiles were tested---cooperative (high agreeableness and conscientiousness, low neuroticism), selfish (low agreeableness and conscientiousness, high neuroticism), leader (high extraversion, conscientiousness, and openness), anxious (low extraversion, high neuroticism, low openness), and average (all domains neutral).

\begin{table}[H]
\centering
\caption{Personality Validation: BFI-2-Expanded Assignment vs.\ Mini-IPIP Measurement}
\label{tab:validation}
\begin{tabular}{lcccccc}
\toprule
Profile & EXT & AGR & CON & NEU & OPE & Match \\
\midrule
Cooperative & 3.00 & \textbf{4.75} & \textbf{4.50} & \textbf{1.00} & 3.50 & 5/5 \\
Selfish & 1.75$^\dagger$ & \textbf{1.00} & \textbf{1.25} & \textbf{5.00} & 3.50 & 4/5 \\
Leader & \textbf{4.75} & 3.50 & \textbf{4.75} & \textbf{1.00} & \textbf{5.00} & 5/5 \\
Anxious & \textbf{1.00} & 3.00 & 2.50 & \textbf{5.00} & \textbf{1.00} & 5/5 \\
Average & 3.00 & 4.00$^\ddagger$ & 2.75 & 3.00 & 4.00$^\ddagger$ & 3/5 \\
\midrule
\multicolumn{6}{l}{Overall alignment} & 22/25 (88\%) \\
\bottomrule
\end{tabular}

\medskip
\footnotesize
\textit{Note.} Bold = assigned extreme (high or low), correctly measured. $^\dagger$ Extraversion pulled low by negative trait descriptions. $^\ddagger$ Upward drift due to RLHF prosociality bias. Domain scores on 1--5 scale; classified as high ($\geq 3.75$), average (2.25--3.75), or low ($\leq 2.25$).
\end{table}

Results showed 88\% domain-level alignment (22/25 domain-profile combinations correctly classified; Table~\ref{tab:validation}). Extreme profiles achieved near-perfect alignment, while the average profile exhibited upward drift in agreeableness and openness---a known artifact of RLHF training that biases models toward prosocial and intellectually curious responses \citep{salecha2024social}.
...
Personality-endowed agents showed dramatically different behavior across all four parts of the experiment. Baseline contribution averaged 13.0 tokens ($SD = 6.2$), with contributions spanning the full 0--20 range. This variance---entirely absent in the default condition---reflects the diversity of personality profiles drawn from population norms.
...
Personality agents showed clear sensitivity to punishment regimes, and the ordering of contributions across regimes is consistent with rational expectations (Figure~\ref{tab:results}):
